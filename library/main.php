...
$catId = false;
    ...
    // Show all products choosen category by default   //echo  json_encode($AjaxFirstShowProduct);
        if (isset($_POST['category'])){
            ...
            $catId =  getCategory ($idCat);
                // Pagination
                $products = 5;
                $pagination = false
                +$pagination = getPage ($idCat);
                if ($pagination == false){
                    $pagination = 1;
                }
                +$pageOffset = ($pagination - 1) * $products;
    // END Pagination END

            $sqlFirstShowProduct = "SELECT prod.id, prod.prod_name, prod.prod_price,  prod.prod_description, img.img_name
                FROM product as prod, `image` as img WHERE cat_id = $catId AND img.prod_id = prod.id ORDER BY id LIMIT $pageOffset, $products";       
    ...
    // // FILTER BY CHARACTERISTIC       echo  json_encode($filter);        
        if (isset($_POST['character']) && isset($_POST['categoryId'])){
            ...
            $typeCharacter = strval ($typeCharacter);
            $catId = +$_POST['categoryId'];
    ...
    // FILTER PAGINATION    //echo  jecho  json_encode($AjaxProductPagin);
        if (isset($_POST['pagination'])){
            $varPagination = $_POST['pagination'];
            +$catId =  getCategory ($varPagination);
            // Pagination
            $products = 5;
            $pagination = false
            +$pagination = getPage ($varPagination);
            if ($pagination == false){
                $pagination = 1;
            }
            $pageOffset = ($pagination - 1) * $products;
    // END Pagination END
            $sqlProductPagin = " SELECT prod.id, prod.prod_name, prod.prod_price,  prod.prod_description, img.img_name 
            FROM product as prod, `image` as img WHERE cat_id = $catId AND img.prod_id = prod.id LIMIT $products, $pageOffset";
            
            $stmtProductPagin = $pdo->query($sqlProductPagin);
                while ($selectProductPagin = $stmtProductPagin->fetch(PDO::FETCH_ASSOC)){
                    $AjaxProductPagin[] = $selectProductPagin;
                }
                //var_dump ($AjaxProductPagin);
            echo  json_encode($AjaxProductPagin);
        }
    // end filter
